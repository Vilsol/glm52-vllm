...
class Glm4MoeModelToolParser(ToolParser):
    ...
    def _extract_tool_name_from_region(
        ...
        For a zero-argument call the name is terminated by ``</tool_call>``
        instead (no ``\\n``/``<arg_key>``), so when the region is complete and
        has no delimiter the whole region is the name. Returns ``None`` if the
        name hasn't fully arrived yet.
        """
        nl = inner_text.find("\n")
        ak = inner_text.find(self.arg_key_start)
        candidates = [i for i in [nl, ak] if i != -1]
        if not candidates:
            # Zero-argument tool call: only safe to emit the name once the
            # region is complete (</tool_call> seen); otherwise it may still
            # be streaming in.
            if is_complete:

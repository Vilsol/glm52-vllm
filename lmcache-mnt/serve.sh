...
if [[ "${GLM52_ENABLE_LMCACHE:-0}" == "1" ]]; then
  ...
  rm -f "${LMCACHE_LOG}"
  # L2 disk adapter is optional: set GLM52_LMCACHE_L2_GB=0 for RAM-only (no disk).
  L2_ARGS=()
  if [[ "${LMCACHE_L2_GB}" != "0" ]]; then
    L2_ARGS=(--l2-adapter "{\"type\":\"fs\",\"base_path\":\"${LMCACHE_DISK_PATH}\",\"relative_tmp_dir\":\"tmp\",\"max_capacity_gb\":${LMCACHE_L2_GB},\"eviction\":{\"eviction_policy\":\"LRU\",\"trigger_watermark\":0.8,\"eviction_ratio\":0.1}}")
  else
    echo "[glm52-v11] LMCache L2 disk DISABLED — RAM-only (L1=${LMCACHE_L1_GB}GB)"
  fi
  lmcache server \
  ...
  _kv_transfer_config=$(printf '{"kv_connector":"LMCacheMPConnector","kv_role":"kv_both","kv_connector_extra_config":{"lmcache.mp.host":"tcp://%s","lmcache.mp.port":%s,"lmcache.mp.mq_timeout":300,"lmcache.mp.heartbeat_interval":5}}' "${LMCACHE_MP_HOST}" "${LMCACHE_MP_PORT}")
  args+=(

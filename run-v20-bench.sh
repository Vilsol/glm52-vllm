...
curl -s http://127.0.0.1:8443/metrics | grep -E '^vllm:spec_decode_num_(accepted|draft)_tokens_total' > "$OUT/mtp-v20-before.txt"
...
LLM_BENCH_MAX_PREFILL=917504 ./.venv/bin/python -u llm_decode_bench.py \
  ...
  --display-mode plain \
  --output "$OUT/bench-v20-full-20260724.json" \
  > "$OUT/bench-v20-full-20260724.log" 2>&1
rc=$?
log "bench exited rc=$rc"

# 5) MTP counters after + accept rate
curl -s http://127.0.0.1:8443/metrics | grep -E '^vllm:spec_decode_num_(accepted|draft)_tokens_total' > "$OUT/mtp-v20-after.txt"
python3 - <<'PY'
import re
def g(f):
    d={}
    ...
    return d
try:
    b=g('/root/glm52-vllm/mtp-v20-before.txt'); a=g('/root/glm52-vllm/mtp-v20-after.txt')
    dd=a['draft']-b['draft']; da=a['accepted']-b['accepted']
    print(f"MTP v20: draft={dd:.0f} accepted={da:.0f} accept_rate={100*da/dd:.2f}%")
